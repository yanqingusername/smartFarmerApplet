...
Page({
    ...
    saveData2: function (e) {
        ...
        request.request_get('/pigManagement/updatePigGeryName.hn', data, function (res) {
            //console.info('回调', res)
            if (res) {
                if (!res.success) {
                    box.showToast(res.msg)
    ...
    get_piggery: function () {
        ...
        request.request_get('/pigManagement/getPiggeryAndSketch.hn', data, function (res) {
            //console.info('回调', res)
            if (res) {
                if (res.success) {
                    var piggery_list = res.msg;
                    var isDisabled2 = [];
                    var focus2 = [];
                    for (let index = 0; index < piggery_list.length; index++) {
                        isDisabled2.push(true);
                        focus2.push(false);
                    }
                    console.log(isDisabled2);
                    that.setData({
                        piggery_list: piggery_list,
    ...
    enter_sty: function (e) {
        ...
        wx.navigateTo({
            url: '/pages/pigOperation/piggery?piggery=' + piggery_number,
        })
    },
